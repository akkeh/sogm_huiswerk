...
int Player::read(const char* filename, int print) {
    int error = -1;
    ...
    in_sig = new short[size];
    data = new float[size];

    //samples:
    for(int n=0; n<size; n++) {
        fread(&in_sig[n], sizeof(short), 1, infile);
        data[n] = double(in_sig[n])/bit_depth;
    }
    ...
    if(error == 0) {
        if(print == 1) printf("\nloaded %s, %i samples\n", filename, size);
        //  write data (can be multi-channel!!!!)
        this->samples = data;
        this->N = size;
...
int Player::write(const char* filename, float* data, unsigned long N, unsigned short bps, int print) {
    int error = -1;
    FILE* outfile=fopen(filename, "wb");
    if(!outfile) { printf("file %s not found", filename);}

    unsigned int ck3_size = N * 2;
